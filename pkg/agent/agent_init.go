...
func registerSharedTools(
	...
	for _, agentID := range registry.ListAgentIDs() {
		...
		if schoolRuntime != nil && schoolRuntime.AdminUpdatesEnabled() {
			agent.Tools.Register(tools.NewRuntimeConfigUpdateTool(schoolRuntime.Store(), schoolRuntime.SchoolStateID(), schoolRuntime.AdminUpdateChannels(), cfg.RuntimeState.ProtectedUpdatePaths))
		}
